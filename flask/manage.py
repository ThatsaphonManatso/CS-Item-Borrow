...
def seed_db():
    ...
    db.session.add(AuthUser(username = "THATSAPHON MANATSO"
    ...
    db.session.add(AuthUser(username = "ANANYA AUPHADEE", student_id = "660510682"
        , email = "ananya_aup@cmu.ac.th", gmail = "ananyamog@gmail.com", is_admin = False, avatar_url = "https://ui-avatars.com/api/?name=A+A&background=d68936&color=2976c9"))
    db.session.add(AuthUser(username = "Kittipitch Kuptavanich"
        , email = "kittipitch.k@cmu.ac.th", is_admin = False, avatar_url = "https://ui-avatars.com/api/?name=K+K&background=d68936&color=2976c9"))
    ...
    db.session.add(AuthUser(username = "Pantawan Sosamer", student_id = "660510664"
        , email = "pantawan_s@cmu.ac.th", gmail = "pantawan0304@gmail.com", is_admin = False, avatar_url = "https://ui-avatars.com/api/?name=P+F&background=d68936&color=2976c9"))
    db.session.add(AuthUser(username = "Ratsameetip Wita"
        , email = "ratsameetip.w@cmu.ac.th", is_admin = False, avatar_url = "https://ui-avatars.com/api/?name=K+K&background=d68936&color=2976c9"))
    bb = BorrowRequest(borrower_id = 2, borrow_date = datetime.now(),
                       return_date = datetime.now(), verifier_id=1, items=[1, 2, 3, 9])
    bc = BorrowRequest(borrower_id = 3, borrow_date = datetime.now(),
                       return_date = datetime.now(), verifier_id=1, items=[4, 5, 7, 10])
    bd = BorrowRequest(borrower_id = 5, borrow_date = datetime.now(),
                       return_date = datetime.now(), verifier_id=1, items=[11, 12, 13, 19])
    bg = BorrowRequest(borrower_id = 2, borrow_date = datetime.now(),
                       return_date = datetime.now(), verifier_id=4, items=[56, 59, 85])
    # bf.update_status("Borrowing")
    # bf.update_status("Borrowing")
    db.session.add(bb)
    db.session.add(bc)
    db.session.add(bd)
